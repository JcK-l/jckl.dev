...
const About = () => {
  ...
  const isMail = isBitSet(BitPosition.FLAG_SECRET);
    ...
    <>
      ...
      >
        <div className="z-10 w-full">
          <h1 className="inline-block mb-8 xl:mb-24 h2-text text-titleColor">
            About Me!
          </h1>
        </div>
        ...
        <PuzzleProvider>
          <div className="relative flex justify-between flex-col-reverse md:flex-row gap-2 mb-1">
            <div className="relative flex flex-col w-full gap-2">
              <AboutText text="" showRange={{min:0, max:3}} removeOnNext={true} isAboutText={!isMail}/>
              <AboutText 
                ...
                showRange={{min:4, max:7}} removeOnNext={!isWide} isAboutText={!isMail}/>
              <AboutText 
                ...
                showRange={{min:8, max:11}} removeOnNext={!isWide} isAboutText={!isMail}/>
              <AboutText 
                ...
                showRange={{min:12, max:16}} removeOnNext={!isWide} isAboutText={!isMail} />
              <Email name={formData.name} email={formData.email} message={formData.message} date={date} isMail={isMail} />
            </div>
            <PuzzleGame />
          </div>
        </PuzzleProvider>
